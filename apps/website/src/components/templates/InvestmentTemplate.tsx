...
export default function InvestmentTemplate({ template, viewport = 'desktop', initialPage = 'home', company, theme: dynamicTheme, projects, posts }: TemplateProps) {
  ...
  const activePosts = posts && posts.length > 0
    ...
    : ((globalThis as any).__newsdata_ref || []);

  // Shadowing variables
  const newsData: any = activePosts;
  ...
  const activeProjects = projects && projects.length > 0
    ...
    : ((globalThis as any).__investment_properties_ref || []);

  // Shadowing variables
  const INVESTMENT_PROPERTIES: any = activeProjects;
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="bg-gray-50 p-8 rounded-2xl border border-gray-100 relative">
                <div className="absolute -top-5 right-8 text-6xl text-blue-100 font-serif">&ldquo;</div>
                <div className="flex gap-1 mb-4">
                  {[...Array(testimonial.rating)].map((_, j) => <Star key={j} size={16} className="fill-yellow-400 text-yellow-400" />)}
                </div>
                <p className="text-gray-600 mb-6 relative z-10 italic">&ldquo;{testimonial.text}&rdquo;</p>
                <div className="flex items-center gap-4">
