...
export default function IndustrialTemplate({ template, viewport = 'desktop', initialPage = 'home', company, theme: dynamicTheme, projects, posts }: TemplateProps) {
  ...
  const activePosts = posts && posts.length > 0
    ...
    : ((globalThis as any).__news_data_ref || []);

  // Shadowing variables
  const NEWS_DATA: any = activePosts;
  ...
  const activeProjects = projects && projects.length > 0
    ...
    : ((globalThis as any).__industrial_properties_ref || []);

  // Shadowing variables
  const INDUSTRIAL_PROPERTIES: any = activeProjects;
