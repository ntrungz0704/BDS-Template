...
import { 
  Waves, Palmtree, Sun, Compass, Anchor, MapPin, Phone, Mail, Search, ChevronRight, ChevronLeft, Menu, X,
...
// ── COLOR PALETTE: OCEAN BLUE & SUNSET GOLD ──────────────────────────────────
const OCEAN = '#0284C7';
const OCEAN_DARK = '#0369A1';
const GOLD = '#F59E0B';

export default function Portal07CoastalResortTemplate({ template, viewport = 'desktop', initialPage = 'home' }: TemplateProps) {
  const isSmall = viewport === 'mobile' || viewport === 'tablet';
  const tSlug = template?.slug || 'portal-07';
    ...
    <header className="bg-white/95 backdrop-blur-md border-b border-sky-100 sticky top-0 z-40 shadow-xs">
      <div className={`${MAX_W} mx-auto px-4 py-3 flex items-center justify-between`}>
        <div onClick={() => navigate('home')} className="flex items-center gap-2.5 cursor-pointer group">
          ...
          <div>
            <div className="text-xl font-black text-[#0284C7] tracking-tight flex items-center gap-1">
              OCEAN<span className="text-[#F59E0B]">ESTATE</span>
            </div>
            <div className="text-[10px] text-sky-600 font-bold uppercase tracking-wider">Coastal & Resort Living</div>
          </div>
        </div>

        <nav className="hidden lg:flex items-center gap-1.5 font-bold text-xs text-slate-700">
          <button onClick={() => navigate('home')} className={`px-3 py-2 rounded-full transition ${currentPage === 'home' ? 'text-[#0284C7] bg-sky-50' : 'hover:text-[#0284C7]'}`}>Trang Chủ Biển</button>
          <button onClick={() => { setFilterCategory('ban'); navigate('sale'); }} className={`px-3 py-2 rounded-full transition ${currentPage === 'sale' ? 'text-[#0284C7] bg-sky-50' : 'hover:text-[#0284C7]'}`}>Biệt Thự Biển Bán</button>
          <button onClick={() => { setFilterCategory('thue'); navigate('rent'); }} className={`px-3 py-2 rounded-full transition ${currentPage === 'rent' ? 'text-[#0284C7] bg-sky-50' : 'hover:text-[#0284C7]'}`}>Thuê Nghỉ Dưỡng</button>
          <button onClick={() => navigate('projects')} className={`px-3 py-2 rounded-full transition ${['projects', 'project-detail'].includes(currentPage) ? 'text-[#0284C7] bg-sky-50' : 'hover:text-[#0284C7]'}`}>Tổ Hợp Nghỉ Dưỡng</button>
          <button onClick={() => navigate('news')} className={`px-3 py-2 rounded-full transition ${['news', 'news-detail'].includes(currentPage) ? 'text-[#0284C7] bg-sky-50' : 'hover:text-[#0284C7]'}`}>Cẩm Nang Đầu Tư Biển</button>
          <button onClick={() => navigate('about')} className={`px-3 py-2 rounded-full transition ${currentPage === 'about' ? 'text-[#0284C7] bg-sky-50' : 'hover:text-[#0284C7]'}`}>Về OceanEstate</button>
  ...
  // ── OCEAN VIEW WIDE CARD (SIGNATURE OF TEMPLATE 07) ────────────────────────
  const renderOceanCard = (item: PortalProperty) => (
    <div
      key={item.id}
      ...
      <div>
        <div className="h-56 relative overflow-hidden bg-sky-50">
          ...
          <div className="absolute top-3 left-3 flex gap-1.5">
            <span className="px-3 py-1 bg-sky-900/85 text-sky-200 text-[10px] font-bold uppercase rounded-full flex items-center gap-1">
              <Waves size={11} className="text-[#F59E0B]" /> Cách Biển 50m
            </span>
          </div>
          <div className="absolute bottom-3 left-3 px-3 py-1 bg-white/95 backdrop-blur-xs rounded-2xl shadow-md">
            <strong className="text-sm font-black text-[#0284C7]">{item.price}</strong>
          </div>
        </div>

        <div className="p-5 space-y-2">
          <span className="text-[11px] font-bold text-[#F59E0B] uppercase block">🏖️ {item.type}</span>
  ...
  const renderHomePage = () => (
    <div className="bg-[#F0F9FF]/50 space-y-16 pb-16">
      ...
      <section className="relative pt-16 pb-28 px-4 bg-gradient-to-b from-[#0284C7] to-[#0369A1] text-white">
        <div className={`${MAX_W} mx-auto text-center max-w-3xl mb-8`}>
          <span className="inline-flex items-center gap-1.5 px-4 py-1 rounded-full bg-white/10 border border-white/20 text-[#F59E0B] text-xs font-bold uppercase tracking-wider mb-4">
            <Sun size={14} /> Thiên Đường Nghỉ Dưỡng & Dòng Tiền Triệu Đô
          </span>
          <h1 className="text-3xl md:text-5xl font-black text-white leading-tight mb-4">
            Sở Hữu Bất Động Sản Ven Biển Đẹp Nhất Việt Nam
          </h1>
          <p className="text-sky-100 text-sm md:text-base font-light max-w-xl mx-auto">
            Hơn 8.000 biệt thự mặt biển, shophouse phố biển và condotel 5 sao quản lý quốc tế.
          </p>
        </div>
          ...
          <div className="bg-white rounded-3xl p-6 shadow-xl border border-sky-100 text-slate-800">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
              <div className="md:col-span-4 relative">
                ...
                <input
                  type="text"
                  placeholder="Tìm theo bãi biển, vịnh, tổ hợp resort..."
                  value={searchKeyword}
              ...
              <div className="md:col-span-3">
                <select value={filterType} onChange={e => setFilterType(e.target.value)} className="w-full py-3 px-3 bg-sky-50/50 border border-sky-200 rounded-2xl text-xs">
                  <option value="all">Tất cả BĐS biển</option>
                  <option value="Biệt thự">Biệt Thự Mặt Biển</option>
                  <option value="Căn hộ chung cư">Condotel 5 Sao</option>
                  <option value="Mặt bằng kinh doanh">Shophouse Phố Biển</option>
                </select>
              </div>
              <div className="md:col-span-3">
                <select value={filterCity} onChange={e => setFilterCity(e.target.value)} className="w-full py-3 px-3 bg-sky-50/50 border border-sky-200 rounded-2xl text-xs">
                  <option value="all">Thủ phủ du lịch biển</option>
                  <option value="Đà Nẵng">Đà Nẵng & Hội An</option>
                  <option value="TP. Hồ Chí Minh">Phú Quốc & Vũng Tàu</option>
                  <option value="Hà Nội">Hạ Long & Vân Đồn</option>
                </select>
              </div>
              <div className="md:col-span-2">
                <button onClick={() => navigate('sale')} className="w-full py-3 bg-[#0284C7] hover:bg-[#0369A1] text-white font-bold text-xs uppercase rounded-2xl shadow-md">
                  Tìm BĐS Biển
                </button>
              </div>
            </div>
      ...
      {/* Featured Ocean Listings */}
      <section className={`${MAX_W} mx-auto px-4 pt-16`}>
        <div className="flex items-center justify-between mb-8">
          <div>
            <span className="text-xs font-bold text-[#0284C7] uppercase tracking-wider">Tuyệt tác mặt biển</span>
            <h2 className="text-2xl md:text-3xl font-black text-[#0369A1]">Bất Động Sản Nghỉ Dưỡng Nổi Bật</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {filteredProperties.slice(0, 6).map(renderOceanCard)}
        </div>
      </section>
    </div>
  );
  ...
  const renderListingCatalogPage = () => (
    <div className="py-8 bg-[#F0F9FF]/50 min-h-screen">
      <div className={`${MAX_W} mx-auto px-4 space-y-6`}>
        <h1 className="text-2xl font-black text-[#0369A1]">Danh Mục Bất Động Sản Nghỉ Dưỡng Biển</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {filteredProperties.map(renderOceanCard)}
        </div>
      </div>
    </div>
  );
  ...
  const renderProjectsPage = () => (
    <div className="py-8 bg-[#F0F9FF]/50 min-h-screen">
      <div className={`${MAX_W} mx-auto px-4 space-y-8`}>
        <h1 className="text-3xl font-black text-[#0369A1]">Tổ Hợp Nghỉ Dưỡng Biển Tiêu Biểu</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {PORTAL_PROJECTS.map(p => (
            <div key={p.id} onClick={() => handleOpenProject(p)} className="bg-white rounded-3xl border border-sky-100 p-4 cursor-pointer shadow-xs hover:shadow-lg transition">
              ...
              <h3 className="font-bold text-base text-[#0284C7]">{p.title}</h3>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="py-8 bg-[#F0F9FF]/50 min-h-screen">
        <div className={`${MAX_W} mx-auto px-4 space-y-4 max-w-4xl`}>
          ...
          <div className="bg-white p-8 rounded-3xl border border-sky-100 space-y-4">
            <h1 className="text-3xl font-black text-[#0284C7]">{selectedProject.title}</h1>
  ...
  const renderNewsPage = () => (
    <div className="py-8 bg-[#F0F9FF]/50 min-h-screen">
      <div className={`${MAX_W} mx-auto px-4 space-y-8 max-w-4xl`}>
        <h1 className="text-3xl font-black text-[#0369A1]">Cẩm Nang Đầu Tư BĐS Biển & Dòng Tiền</h1>
      ...
      <div className="py-8 bg-[#F0F9FF]/50 min-h-screen">
        <div className={`${MAX_W} mx-auto px-4 space-y-4 max-w-4xl`}>
          ...
          <article className="bg-white p-8 rounded-3xl border border-sky-100 space-y-4">
            <h1 className="text-3xl font-black text-[#0284C7]">{selectedNews.title}</h1>
  ...
  const renderAboutPage = () => (
    <div className="py-8 bg-[#F0F9FF]/50 min-h-screen">
      <div className={`${MAX_W} mx-auto px-4 max-w-3xl text-center bg-white p-8 rounded-3xl border border-sky-100 space-y-4`}>
        <h1 className="text-3xl font-black text-[#0284C7]">Về OceanEstate</h1>
        <p className="text-sm text-slate-600">Đơn vị tư vấn và phân phối bất động sản nghỉ dưỡng biển, biệt thự mặt biển hàng đầu Việt Nam.</p>
      </div>
    </div>
  );
  ...
  const renderContactPage = () => (
    <div className="py-8 bg-[#F0F9FF]/50 min-h-screen">
      <div className={`${MAX_W} mx-auto px-4 max-w-2xl bg-white p-8 rounded-3xl border border-sky-100 space-y-4`}>
        <h1 className="text-2xl font-black text-[#0284C7]">Liên Hệ & Ký Gửi BĐS Biển</h1>
        <form onSubmit={e => { e.preventDefault(); setContactSubmitted(true); }} className="space-y-3 text-xs">
          ...
          <textarea rows={4} required placeholder="Thông tin biệt thự hoặc condotel biển..." className="w-full p-3 border rounded-2xl" />
          <button type="submit" className="w-full py-3 bg-[#0284C7] text-white font-bold rounded-2xl">Gửi Yêu Cầu</button>
        </form>
      </div>
    </div>
  );
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={() => setIsAuthModalOpen(false)}>
        <div className="bg-white rounded-3xl max-w-md w-full p-8 relative" onClick={e => e.stopPropagation()}>
          ...
          <h3 className="text-lg font-bold text-center mb-4 text-[#0284C7]">Tài Khoản OceanEstate</h3>
  ...
  const renderFooter = () => (
    <footer className="bg-[#0369A1] text-white pt-8 pb-6 border-t border-sky-800 text-xs">
      <div className={`${MAX_W} mx-auto px-4 text-center space-y-2 text-sky-200`}>
        <div className="font-bold text-white uppercase tracking-wider">OCEANESTATE — THIÊN ĐƯỜNG NGHỈ DƯỠNG BIỂN VIỆT NAM</div>
        <p>© 2026 CloneCraft PlatformBDS — Template #07 Coastal & Resort Living Portal.</p>
      </div>
    </footer>
  );
