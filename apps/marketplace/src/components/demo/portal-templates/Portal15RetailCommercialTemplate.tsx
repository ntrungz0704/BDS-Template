  ...
  const renderHeader = () => (
    <header className="bg-white border-b border-orange-200 sticky top-0 z-40 shadow-xs">
      ...
      <div className={`${MAX_W} mx-auto px-4 py-3 flex items-center justify-between`}>
        ...
        <button onClick={() => navigate('contact')} className="px-4 py-2 bg-[#EA580C] hover:bg-[#C2410C] text-white font-bold text-xs rounded-xl shadow flex items-center gap-1.5">
          <Store size={14} /> Ký Gửi Mặt Bằng
        </button>
      </div>
    </header>
  );
