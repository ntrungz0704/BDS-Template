...
export default function BDS17Template({
  ...
  const activeUnits = useMemo<UnitItem[]>(() => {
    if (projects && Array.isArray(projects) && projects.length > 0) {
      return projects.map((p: any, idx: number): UnitItem => ({
        ...
        planImage: p.floorplan || p.image || 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=800&q=80',
        hot: Boolean(idx === 0),
  ...
  const [selectedUnit, setSelectedUnit] = useState<UnitItem>(() => {
    ...
    return activeUnits[0] || BDS17_UNITS[0];
  });
  ...
  const [selectedArticle, setSelectedArticle] = useState<NewsItem>(() => {
    ...
    return activeNews[0] || BDS17_NEWS[0];
  });
  ...
  const availableFloors = useMemo(() => {
    const set = new Set(activeUnits.map(p => p.floor).filter(Boolean));
    return ['all', ...Array.from(set)];
  }, [activeUnits]);
