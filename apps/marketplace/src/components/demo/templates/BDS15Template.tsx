...
export default function BDS15Template({
    ...
    if (projects && Array.isArray(projects) && projects.length > 0) {
      const customProps = projects.map((p: any, idx: number): PropertyItem => {
        ...
        return {
          ...
          title: p.title || p.name || 'Bất động sản trung tâm phố cổ',
          slug: p.slug || `bds-${idx + 1}`,
          type: p.type || 'Nhà Mặt Phố',
          category: cat,
          price: p.price || (p.priceFrom ? `${p.priceFrom} Tỷ` : 'Liên hệ'),
          priceNum: typeof p.priceNum === 'number' ? p.priceNum : (parseFloat(p.price) || 20.0),
          area: typeof p.area === 'number' ? `${p.area} m²` : (p.area || '100 m²'),
          areaNum: typeof p.area === 'number' ? p.area : 100,
          beds: p.beds || p.bedrooms || 4,
          baths: p.baths || p.bathrooms || 4,
          location: p.address || p.location || 'Vị trí mặt phố kinh doanh đắc địa',
          ...
          description: p.description || p.desc || 'Vị trí kim cương trung tâm phố lớn, dòng tiền cho thuê khủng.',
          specs: Array.isArray(p.specs) ? p.specs : ['Sổ đỏ chính chủ', 'Mặt tiền rộng', 'Dòng tiền sẵn'],
        };
      });
      const customSlugs = new Set(customProps.map((cp: any) => cp.slug));
      const remainingDefaults = (BDS15_PROPERTIES).filter((dp: any) => !customSlugs.has(dp.slug));
      return [...customProps, ...remainingDefaults];
    }
    return BDS15_PROPERTIES;
  }, [projects]);
  ...
  const activeNews = useMemo<NewsItem[]>(() => {
    if (posts && Array.isArray(posts) && posts.length > 0) {
      const customNews = posts.map((p: any, idx: number): NewsItem => ({
        id: p.id || idx + 1,
        title: p.title || 'Tin tức bất động sản phố cổ',
      ...
      const remainingDefaults = (BDS15_NEWS).filter((dn: any) => !customSlugs.has(dn.slug));
  ...
  const [selectedProperty, setSelectedProperty] = useState<PropertyItem>(() => {
    ...
    return activeProperties[0] || BDS15_PROPERTIES[0];
  });
  ...
  const [selectedArticle, setSelectedArticle] = useState<NewsItem>(() => {
    ...
    return activeNews[0] || BDS15_NEWS[0];
  });
  ...
  const availableDistricts = useMemo(() => {
    const set = new Set(activeProperties.map(p => p.district).filter(Boolean));
    return ['all', ...Array.from(set)];
  }, [activeProperties]);
    ...
    <section className="relative bg-slate-950 text-white overflow-hidden min-h-[380px] sm:min-h-[460px] flex items-center justify-center">
      ...
      <div className="relative z-20 text-center space-y-4 max-w-2xl mx-auto px-4">
        <div className="inline-block bg-[#0D9488]/95 backdrop-blur-md px-10 py-7 shadow-2xl border border-teal-200/40 space-y-2">
          ...
          <div className="pt-2">
            <button
              onClick={() => handleOpenProperty(activeProperties[4] || activeProperties[0] || BDS15_PROPERTIES[4])}
  ...
  const render3SpecialBanners = () => (
    <section className="py-8 bg-white px-4">
      <div className={`${MAX_W} mx-auto grid grid-cols-1 md:grid-cols-3 gap-6`}>
        ...
        <div 
          onClick={() => handleOpenProperty(activeProperties[1] || BDS15_PROPERTIES[1])}
          className="relative aspect-[16/9] overflow-hidden shadow-md group cursor-pointer border border-slate-200"
        >
        ...
        <div 
          onClick={() => handleOpenProperty(activeProperties[0] || BDS15_PROPERTIES[0])}
          className="relative aspect-[16/9] overflow-hidden shadow-md group cursor-pointer border border-slate-200"
        >
