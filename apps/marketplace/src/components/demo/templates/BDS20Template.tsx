...
export default function BDS20Template({
  ...
  const activeUnits = useMemo<UnitItem[]>(() => {
    if (projects && Array.isArray(projects) && projects.length > 0) {
      return projects.map((p: any, idx: number): UnitItem => ({
        ...
        block: p.block || 'Block A - Park View',
        type: p.type || '2 Phòng Ngủ',
        floor: p.floor || 'Tầng 10',
        price: p.price || (p.priceFrom ? `Từ ${p.priceFrom} Tỷ` : 'Liên hệ'),
        priceNum: typeof p.priceNum === 'number' ? p.priceNum : (parseFloat(p.price) || 3.6),
        ...
        view: p.view || 'View Công Viên Xanh & Hồ Cảnh Quan',
  ...
  const [selectedUnit, setSelectedUnit] = useState<UnitItem>(() => {
    ...
    return activeUnits[0] || BDS20_UNITS[0];
  });
  ...
  const [selectedArticle, setSelectedArticle] = useState<NewsItem>(() => {
    ...
    return activeNews[0] || BDS20_NEWS[0];
  });

  // Dynamic Options derived from Data for 100% CMS Resilience
  const availableBlocks = useMemo(() => {
    const set = new Set(activeUnits.map(p => p.block).filter(Boolean));
