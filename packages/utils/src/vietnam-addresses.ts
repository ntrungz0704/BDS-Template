/**
 * Dữ liệu hành chính ĐẦY ĐỦ 63 Tỉnh / Thành phố, Toàn bộ Quận / Huyện & Phường / Xã Việt Nam
 * Nguồn: Tổng cục Thống kê (GSO) & Dữ liệu ranh giới hành chính chuẩn Việt Nam
 */
...
export function getDistricts(provinceName: string): string[] {
  ...
  const prov = VIETNAM_PROVINCES.find((p) => {
...
export function getWards(provinceName: string, districtName: string): string[] {
  ...
  const dist = prov.districts.find((d) => {
...
/**
 * Tách địa chỉ đầy đủ thành 4 phần: Số nhà/Đường, Phường/Xã, Quận/Huyện, Tỉnh/Thành phố
 */
export function parseAddress(fullAddress: string): {
  ...
  if (!fullAddress || fullAddress === 'Chưa cập nhật') {
...
/**
 * Ghép các trường địa chỉ thành chuỗi hoàn chỉnh
 */
export function formatAddress(street: string, ward: string, district: string, province: string): string {
  const parts = [street, ward, district, province].filter((p) => Boolean(p && p.trim()));
  return parts.join(', ');
}
