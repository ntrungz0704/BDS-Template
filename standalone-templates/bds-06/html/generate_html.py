    ...
    <header class="sticky top-0 z-40 bg-[#0F172A] text-white shadow-xl border-b border-slate-800">
      ...
      <div id="mobile-menu" class="hidden lg:hidden bg-slate-900 border-b border-slate-800 px-6 py-5 space-y-3">
        ...
        <div class="pt-2 border-t border-slate-800 flex items-center justify-between text-xs">
          ...
          <a href="tel:0919006030" class="text-red-400 font-extrabold">0919 006 030</a>
        </div>
      </div>
    </header>
  `;
...
// Ensure string builder pattern continues for the rest...

document.addEventListener('DOMContentLoaded', () => {
  const root = document.getElementById('root');
  // ... rest of the code is long, so we'll construct it in JS
});
"""
...
OUTPUT_DIR = r"e:\BĐS Template\standalone-templates\bds-06\html"
...
<body class="bg-[#F8FAFC] text-slate-900 font-sans selection:bg-red-500 selection:text-white flex flex-col min-h-screen">
  ...
  <script>
    ...
    let loanYears = 20;
    let loanRate = 7.5;
    let loanPropertyPrice = 3.65;
    
    function toggleMobileMenu() {
      mobileMenuOpen = !mobileMenuOpen;
      document.getElementById('mobile-menu').style.display = mobileMenuOpen ? 'block' : 'none';
      document.getElementById('menu-icon').innerHTML = mobileMenuOpen ? getIcon('x', 20) : getIcon('menu', 20);
      initIcons();
    }
    ...
    function openLeadModal(title) {
      document.getElementById('lead-modal-title').innerText = title;
    ...
    function handleLeadSubmit(e) {
      e.preventDefault();
      const phone = e.target.querySelector('input[type="tel"]')?.value || document.getElementById('modal-phone')?.value;
      if (!phone) {
        alert('Vui lòng nhập số điện thoại!');
        return;
      }
      
      const API_URL = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://localhost:5000' : 'https://bds-template-api.onrender.com';
      ...
      showToast('🎉 Tiếp nhận yêu cầu! Bảng giá & tài liệu sẽ gửi qua Zalo trong 3 phút!');
      closeLeadModal();
      e.target.reset();
    }
    ...
    function handleConsignSubmit(e) {
      e.preventDefault();
      showToast('✅ Đã tiếp nhận hồ sơ ký gửi BĐS. Chuyên viên sẽ liên hệ thẩm định miễn phí trong 15 phút!');
      e.target.reset();
    }

    function setActiveMasterplanTab(tab) {
      activeMasterplanTab = tab;
      renderApp();
    }
    ...
    function navigate(page, slug) {
      ...
      if (page === 'property-detail' && slug) {
        selectedProperty = BDS06_PROPERTIES.find(p => p.slug === slug) || BDS06_PROPERTIES[0];
        loanPropertyPrice = selectedProperty.priceNum;
      } else if (page === 'news-detail' && slug) {
        selectedArticle = BDS06_NEWS.find(a => a.slug === slug) || BDS06_NEWS[0];
      }
    ...
    function handleOpenProperty(id) {
      const prop = BDS06_PROPERTIES.find(p => p.id === id);
      if (prop) navigate('property-detail', prop.slug);
    }

    function handleOpenArticle(id) {
      const art = BDS06_NEWS.find(a => a.id === id);
      if (art) navigate('news-detail', art.slug);
    }

    function updateMortgage() {
      loanPercent = parseFloat(document.getElementById('loanPercent').value);
      loanYears = parseFloat(document.getElementById('loanYears').value);
          ...
          <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-12">
              ...
              <div id="booking-lead-form" class="lg:col-span-5 bg-[#0F172A] text-white rounded-md p-6 sm:p-8 shadow-2xl border border-slate-800 space-y-4">
                ...
                <form onsubmit="handleLeadSubmit(event)" class="space-y-3">
                  <div>
                  ...
                  <div>
                    ...
                    <input type="tel" required placeholder="Ví dụ: 0919 006 030" class="w-full px-3.5 py-2.5 rounded-sm bg-slate-800 border border-slate-700 text-white text-xs font-bold text-red-400 focus:outline-none focus:border-red-500" />
                  </div>
                  <div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
              <div class="lg:col-span-8 space-y-6">
                <div class="bg-white rounded-md p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
                  ...
                  <div class="relative aspect-video rounded-md overflow-hidden bg-slate-100 shadow-sm">
                    <img src="\${p.image}" class="w-full h-full object-cover" />
                  </div>

                  <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 rounded-sm bg-slate-50 border border-slate-200 text-center">
            ...
            <form onsubmit="handleConsignSubmit(event)" class="bg-white rounded-md p-6 sm:p-10 border border-slate-200 shadow-sm space-y-4">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="tel" required placeholder="Ví dụ: 0919 006 030" class="w-full px-4 py-2.5 rounded-sm border border-slate-300 text-xs font-bold text-red-600 focus:outline-none focus:border-red-500" />
                </div>
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
...
print(f"HTML file successfully written to {FILE_PATH}")
"""

script_path = r"e:\BĐS Template\standalone-templates\bds-06\html\generate_html.py"
