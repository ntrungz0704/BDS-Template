...
2. Copy toàn bộ thư mục `php` này vào thư mục `htdocs` của XAMPP (VD: `C:/xampp/htdocs/bds-01`).
3. Mở **phpMyAdmin** (`http://localhost/phpmyadmin`).
4. Tạo database mới tên: `bds_luxury_gold`.
5. Chọn tab **Import (Nhập)** và chọn file `database.sql` nằm trong thư mục này để nạp dữ liệu.
6. Mở trình duyệt và truy cập: `http://localhost/bds-01` để xem website hoạt động!
